...
pub const COPULA: &[&str] =
    &["be", "am", "is", "are", "was", "were", "been", "being", "become", "became", "seem", "seemed", "remain", "remained"];
...
pub fn verb_class(lemma: &str) -> VerbClass {
    ...
    match lemma {
        ...
        "go" | "come" | "arrive" | "leave" | "return" | "travel" | "walk" | "run" | "enter" | "visit" | "move" | "fly"
        | "ride" | "drive" | "wander" | "flee" | "reach" | "follow" | "cross" | "flow" => Motion,
        ...
        "have" | "own" | "possess" | "inherit" | "receive" | "buy" | "sell" | "keep" | "lose" | "give" | "get" | "win" => {
            Possession
        }
    ...
    for suf in ["ing", "ed"] {
        if let Some(b) = strip(&w, suf) {
            ...
            if bytes.len() >= 3
                ...
                && !b"aeiouls".contains(&bytes[bytes.len() - 1])
            {
    ...
    IRREGULAR.iter().any(|(b, p, pp)| l == *b || l == *p || l == *pp)
        || COMMON_VERBS.iter().any(|v| {
            l == *v
                ...
                || l == format!("{v}ed")
                || l == format!("{v}d")
                || l == format!("{}ied", v.trim_end_matches('y'))
                || l == format!("{v}ing")
                || l == format!("{}ing", v.trim_end_matches('e'))
        })
}
