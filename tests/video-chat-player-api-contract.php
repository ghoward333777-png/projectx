...
$assert($s === 200 && $b['kind'] === 'youtube-playlist' && $b['list'] === 'PL9oaxTKWGJ7ONeR1-1cMkCAGAl-gozWij' && $b['src'] === $b['list'], 'GET /v1/resolve recognises playlists as playable items');
...
[$s, $b] = $call('POST', "/v1/rooms/{$room}/playlist", [], ['url' => 'https://www.youtube.com/playlist?list=PL9oaxTKWGJ7ONeR1-1cMkCAGAl-gozWij'], $auth);
$assert($s === 201 && $b['item']['kind'] === 'youtube-playlist' && $b['item']['src'] === 'PL9oaxTKWGJ7ONeR1-1cMkCAGAl-gozWij', 'a YouTube playlist link becomes one playable item');
$listItem = $b['item']['id'];
[$s, $b] = $call('POST', "/v1/rooms/{$room}/playlist", [], ['url' => 'https://www.youtube.com/playlist?list=PL9oaxTKWGJ7ONeR1-1cMkCAGAl-gozWij', 'expand' => true], $auth);
$assert($s === 201 && ($b['needsImport'] ?? false) === true && $b['list'] === 'PL9oaxTKWGJ7ONeR1-1cMkCAGAl-gozWij', 'expand=1 asks the browser to import the ids instead');
[$s, $b] = $call('PATCH', "/v1/rooms/{$room}/playlist", [], ['repeat' => 'sideways'], $auth + ['x-host-token' => $host]);
$assert($s === 400, 'invalid repeat mode → 400');
[$s, $b] = $call('PATCH', "/v1/rooms/{$room}/playlist", [], ['repeat' => 'all', 'shuffle' => true], $auth + ['x-host-token' => $host]);
$assert($s === 200 && $b['playlist']['repeat'] === 'all' && $b['playlist']['shuffle'] === true && count($b['playlist']['order']) === count($b['playlist']['items']) && $b['playlist']['order'][0] === $b['playlist']['current'], 'host sets repeat-all and shuffle; the order starts with the current item');
[$s, $b] = $call('PATCH', "/v1/rooms/{$room}/playlist", [], ['shuffle' => false], $auth + ['x-host-token' => $host]);
$assert($s === 200 && $b['playlist']['shuffle'] === false && $b['playlist']['order'] === array_column($b['playlist']['items'], 'id'), 'shuffle off restores the natural order');
[$s, $b] = $call('DELETE', "/v1/rooms/{$room}/playlist/{$listItem}", [], [], $auth + ['x-host-token' => $host]);
$assert($s === 200, 'playlist item can be removed');
[$s, $b] = $call('PATCH', "/v1/rooms/{$room}", [], ['youtubeEngine' => 'lite'], $auth + ['x-host-token' => $host]);
$assert($s === 200 && $b['room']['youtubeEngine'] === 'lite', 'PATCH room sets the YouTube engine');
[$s, $b] = $call('PATCH', "/v1/rooms/{$room}", [], ['youtubeEngine' => 'bogus'], $auth + ['x-host-token' => $host]);
$assert($s === 200 && $b['room']['youtubeEngine'] === 'lite', 'unknown engine values are ignored');
